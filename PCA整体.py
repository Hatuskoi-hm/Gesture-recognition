            ...
            for i in range(len(all_lines) - 9):
                ...
                for j in range(5):
                    ...
                    for k in range(i, i + 10):
                        if max_num < all_lines[k][j]:
                            max_num = all_lines[k][j]
                        if min_num > all_lines[k][j]:
                            max_num = all_lines[k][j]
                    input.append(max_num - min_num)
                    """
...
# ax.set_title(f"测试者{index+1}训练集可视化")
# ax.legend()
pname = "PCA_R/" + '2D训练集' + "整体" + '.png'
...
datas = []
labels = []
for index, name in enumerate(['胡宁', '卢天翼', '吴梓浩', '刘宝洋', '孙萌', '孙帼一', '周宇炫', '白泽杨', '陈语娴']):
    file_list = ['1.txt', '2.txt', '3.txt', '4.txt', '5.txt']
    for cur_file in file_list[:5]:
...
ax.set_xlabel('特征1', fontdict={"family": "SimHei", "size": font_s})
ax.set_ylabel('特征2', fontdict={"family": "SimHei", "size": font_s})
# ax.set_title(f"测试者{index+1}测试集可视化")
# ax.legend()
pname = "PCA_R/" + '2D测试集' + "整体" + '.png'
plt.savefig(pname, dpi=1200, bbox_inches='tight')
